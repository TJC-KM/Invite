...
const 預設文案 = {
  ...
  //
  // 四段，刻意壓短過。這段字會擋在表單前面，長了就沒有人讀完，
  // 而沒讀完的人不會知道「不寫也可以」——那才是最該傳達到的一句
  邀請說明: [
    "有些事情，如果沒有記下來，就真的過去了。當下都很真實，可是五年十年之後，連自己都會想不起來。這邊想做的很單純：把這些記下來、保存好，有點像時光膠囊。將來出週年刊、安排見證分享的時候，才有機會來跟你邀稿。",
    "不寫也沒關係，真的。我們不會沒事把大家寫的東西拿出來看；放著就放著，沒有人會來催，也沒有期限。",
    ...
    "不用特地回頭整理一輩子，寫最近的感動就好，現在寫、以後再寫都可以。手上已經有完整一篇的——講台上分享過的也算——直接傳上來就好。真的很謝謝你願意。願神帶領我們每一個人。",
  ].join("\n\n"),

  // 放在頁首標題底下，像一段題詞。整頁存在的理由就是怕忘記，
  // 這節直接說「不可忘記」，後半句又接上標題的「恩典」。
  // 最後一行當出處，字小一號。留白就整塊不顯示
  // 一句一行，照經文本身的句讀斷。交給瀏覽器自己斷的話，
  // 手機上會把「耶和華」拆成兩行
  經文: "我的心哪，你要稱頌耶和華！\n不可忘記他的一切恩惠！\n詩篇 103:2",
...
// 設定檔裡的經文：最後一行當出處，其餘照原本的換行排。只有一行就整段當經文
function 畫經文(值) {
  const 行 = String(值 || "").split("\n").map((x) => x.trim()).filter(Boolean);
  if (!行.length) return "";
  const 出處 = 行.length > 1 ? 行.pop() : "";
  return `<div class="verse">${行.map(esc).join("<br>")}${
