// 邀請卡：卡片、公開活動頁、維護介面、我要參加、Drive 圖片代理。
//
...
// 一張卡片。查無此人或已停用回 null，讓路由去決定要給什麼 404
export async function 卡片(code, request, env, ctx) {
  const invite = await loadInvite(code, env);
  if (!invite || invite.狀態 === "已停用") return null;
...
export async function eventPage(代號, env) {
...
export async function api(動作, request, url, env) {
...
export async function rsvp(request, env) {
...
export async function thumbProxy(fileId, env, ctx) {
...
export async function imageProxy(fileId, env, ctx) {
  if (!/^[\w-]{10,}$/.test(fileId)) return new Response("Not found", { status: 404 });
